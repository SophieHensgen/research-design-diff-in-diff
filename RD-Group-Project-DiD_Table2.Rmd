...
part_time_len <- length(NJ_df$y_part.emp)
managers_len <- length(NJ_df$y_managers)
ompleterecords <- na.omit(NJ_df_before) 
...
part_time_len <- length(PA_df$y_part.emp)
managers_len <- length(PA_df$y_managers)
ompleterecords <- na.omit(PA_df_before) 
...
ompleterecords <- na.omit(NJ_df2)
NJ_df_wage.45 <- ompleterecords[ompleterecords == 4.25]
percent_wage_45 <- (length(NJ_df_wage.45)/length(NJ_df2)) *100
...
ompleterecords <- na.omit(PA_df2)
PA_df_wage.45 <- ompleterecords[ompleterecords == 4.25]
percent_wage_45 <- (length(PA_df_wage.45)/length(PA_df2)) *100
...
part_time_len <- length(NJ_df$y_part.emp.sec)
managers_len <- length(NJ_df$y_managers.sec)
ompleterecords <- na.omit(NJ_df_after) 
...
part_time_len <- length(PA_df$y_part.emp.sec)
managers_len <- length(PA_df$y_managers.sec)
ompleterecords <- na.omit(PA_df_after) 
...
percent_wage.50 <- (length(NJ_df_wage.50)/length(NJ_df2)) *100
...
percent_wage.50 <- (length(PA_df_wage.50)/length(PA_df2)) *100
...
nhi bhai, apna chuck ,,, mera dil ......kuch khany ka dil kr rha h
####################################################################################################
...
data_long <- gather(df3, time, FTE, y_fte.before:y_fte.after, factor_key=TRUE)

data_long$time <- gsub("y_fte.before", 0, data_long$time)
data_long$time <- gsub("y_fte.after", 1, data_long$time)
data_long
...
gen.matched <- GenMatch(X = cbind(data_long$x_bk, data_long$x_kfc, data_long$x_roys, data_long$x_wendys),
                        Tr = data_long$d_nj)

gen.results <- Match(X = cbind(data_long$x_bk, data_long$x_kfc, data_long$x_roys, data_long$x_wendys),
               Tr = data_long$d_nj,
               Weight.matrix = gen.matched)

# 2. Check balance

MatchBalance(data_long$d_nj ~ data_long$x_bk + data_long$x_kfc + data_long$x_roys + data_long$x_wendys,
            match.out = gen.results,
            data = data_long)
...
matching.exact <- Match(X = cbind(data_long$x_bk, data_long$x_kfc, data_long$x_roys, data_long$x_wendys),
               Tr = data_long$d_nj,
               exact = TRUE,
               estimand = "ATT")
...
MatchBalance(d_nj ~ x_bk + x_kfc + x_roys + x_wendys,
...
summary(matching.exact) # same results
summary(gen.results)
```

...
df[,12:14] <- lapply(df[,12:14], as.factor) #first convert into factor, and then into numeric. if tried to convert directly from 
df[,12:14] <- lapply(df[,12:14], as.numeric) #character into numeric an error will appear.

df[,15] <- lapply(df[,15], as.numeric) # sometimes it is better to convert directly as it can affect the values
...
# looking ta the structure of the variables - Check if convertion worked
str(df)
...
# Replace all . with NA, as there is no observed value
...
count.w1$wave2 <- c("Number Interview", "Number of refusals", "Number of Stores in Sample Frame","Response Rate (percentage)") # add variable with labels
...
count.w2.c1$group<- c("Refused", "Answered", "Renovations", "Closed Permanently", "Construction", "Mall fire") # add new variable with labels
...
count.w2.c2$group<- c("Refused", "Answered", "Renovations", "Closed Permanently", "Construction", "Mall fire") # add new variable with labels
...
count.w2.c3$group<- c("Answered", "Closed Permanently") # add new variable with labels
...
count.w2 %>% add_row(group = "Sample", All = 410, NJ = 331, PA = 79) #add row with total in sample
...
total_NJ <- nrow(NJ_df)
total_BK <- nrow (NJ_df[NJ_df$x_store.name ==1,]) # BK in NJ
...
mean(NJ_df$y_fte.before)
df$y_fte.after <- df$y_part.emp.sec*0.5 + df$y_full.emp.sec + df$y_managers.sec 

# FTE before
for(i in 1:nrow(NJ_df)){
  NJ_df$y_fte.before[i] <- NJ_df$y_full.emp[i] + # Full-time employees
    0.5*(NJ_df$y_part.emp[i]) + # plus part-time employees counted as 0.5
    NJ_df$y_managers # plus number of managers
}

# FTE after
for(i in 1:nrow(NJ_df){
  NJ_df$y_fte.after[i] <- NJ_df$y_full.emp.sec[i] +
    0.5*(NJ_df$y_part.emp.sec[i]) +
    NJ_df$y_managers.sec[i]
}

NJ.before <- mean(NJ_df$y_fte.before , na.rm=TRUE)
NJ.after <- mean(NJ$y_fte.after, na.rm=TRUE)


#cdf$y_fte <- df$y_full.emp + df$y_managers + df$y_part.emp*0.5
#mean(df$y_fte) # check separately for NJ and PA

# add new variable and have all 3 
#FTE <- mean(NJ_df$y_full.emp+NJ_df$y_part.emp*0.5+NJ_df$y_managers)

#mean(NJ_df$x_st.wage.before)
#mean(PA_df$x_st.wage.before)

#mean(NJ_df$y_full.emp)

#NJ_df %>% 
 # select(NJ_df$y_full.emp) %>% 
  #group_by(NJ_df$y_full.emp) %>% 
  #summarise(total=n()) %>% 
  #mutate(Percent_NJ=total/sum(total)) %>%  
  #arrange(desc(total)) %>% 
  
 #mean(Percent_NJ) 


# mean(NJ_df[NJ_df$x_store.name ==1,]$y_full.emp + NJ_df[NJ_df$x_store.name ==1,]$y_part.emp * 0.5 +NJ_df[NJ_df$x_store.name ==1,]$y_managers)

#mean(NJ_df[NJ_df$x_store.name ==2,]$y_full.emp + NJ_df[NJ_df$x_store.name ==2,]$y_part.emp * 0.5 +NJ_df[NJ_df$x_store.name ==2,]$y_managers)

...
count.w1.r3 <- dplyr::select(count.w1.r3, -1) # delete status variable
...
count.w1.r4 <- dplyr::select(count.w1.r4, -3) # delete status variable
...
count.w1$wave1 <- c("Number Interview", "Response Rate (percentage)", "Number of Stores in Sample Frame","Number of refusals") # add variable with labels
...
count.w2 <- dplyr::select(count.w2, -1) # delete status variable
...
NJ_df <- df[df$d_state == 1,] # NJ
total_NJ <- nrow(NJ_df) #count rows
total_BK <- nrow (NJ_df[NJ_df$x_store.name ==1,]) #Burger King in NJ
percent_NJ_BK <- total_BK/total_NJ *100 # calculate percentage

total_NJ <- nrow(NJ_df) #count rows
total_KFC <- nrow (NJ_df[NJ_df$x_store.name ==2,]) #KFC in NJ
percent_NJ_KFC <- total_KFC/total_NJ *100 # calculate percentage

total_NJ <- nrow(NJ_df) #count rows
total_roys <- nrow (NJ_df[NJ_df$x_store.name ==3,]) #roys in NJ
percent_NJ_roys <- total_roys/total_NJ *100 # calculate percentage

total_NJ <- nrow(NJ_df) #count rows
total_wendys <- nrow (NJ_df[NJ_df$x_store.name ==4,]) #wendys in NJ
percent_NJ_wendys <- total_wendys/total_NJ *100 # calculate percentage

PA_df <- df[df$d_state == 0,] # PA 
total_PA <- nrow(PA_df) #count rows
total_BK <- nrow (PA_df[PA_df$x_store.name ==1,]) # BK in PA
percent_PA_BK <- total_BK/total_PA *100 # calculate percentage

total_PA <- nrow(PA_df) #count rows
total_KFC <- nrow (PA_df[PA_df$x_store.name ==2,]) #KFC in PA
percent_PA_KFC <- total_KFC/total_PA *100 # calculate percentage

total_PA <- nrow(PA_df) #count rows
total_roys <- nrow (PA_df[PA_df$x_store.name ==3,]) #roys in PA
percent_PA_roys <- total_roys/total_PA *100 # calculate percentage

total_PA <- nrow(PA_df) #count rows
total_wendys <- nrow (PA_df[PA_df$x_store.name ==4,]) #wendys in PA
percent_PA_wendys <- total_wendys/total_PA *100 # calculate percentage
...
NJ_df$y_fte.before <- NJ_df$y_full.emp + NJ_df$y_managers + NJ_df$y_part.emp*0.5


NJ_df$y_fte.after <- NJ_df$y_part.emp.sec*0.5 + NJ_df$y_full.emp.sec + NJ_df$y_managers.sec 


PA_df$y_fte.before <- PA_df$y_full.emp + PA_df$y_managers + PA_df$y_part.emp*0.5


PA_df$y_fte.after <- PA_df$y_part.emp.sec*0.5 + PA_df$y_full.emp.sec + PA_df$y_managers.sec 
...
# 1st wave 
NJ.before <- mean(NJ_df$y_fte.before , na.rm=TRUE) # FTE 1st wave NJ
PA.before <- mean(PA_df$y_fte.before , na.rm=TRUE) # FTE 1st wave PA
NJ.before_wage <- mean(NJ_df$x_st.wage.before , na.rm=TRUE) # Starting wage 1st wave NJ
PA.before_wage <- mean(PA_df$x_st.wage.before , na.rm=TRUE) # Starting wage 1st wave PA
NJ.before_openday <- mean(NJ_df$x_open.per.day , na.rm=TRUE) # Open hours 1st wave NJ
PA.before_openday <- mean(PA_df$x_open.per.day , na.rm=TRUE) # Open hours 1st wave PA
NJ.before_bonus <- mean(NJ_df$x_cash.bounty , na.rm=TRUE)*100 # Recruiting bonus 1st wave NJ
PA.before_bonus <- mean(PA_df$x_cash.bounty , na.rm=TRUE)*100 # Recruiting bonus 1st wave PA
NJ.before_Pfull <- mean(NJ_df$x_medium.soda +NJ_df$x_small.fries +NJ_df$x_entree, na.rm=TRUE) # Price of full meal 1st wave NJ
PA.before_Pfull <- mean(PA_df$x_medium.soda +PA_df$x_small.fries +PA_df$x_entree, na.rm=TRUE) # Price of full meal 1st wave PA

###################################################################################################

# Percentage full-time employees in NJ 1st wave
NJ_df <- df[df$d_state == 1,] # NJ
NJ_df_before<- NJ_df$y_full.emp
part_time_len <- length(NJ_df$y_part.emp) # length of variable
managers_len <- length(NJ_df$y_managers) # length of variable
ompleterecords <- na.omit(NJ_df_before) # delete omits
NJ_df_per.1 = length(ompleterecords)/(length(NJ_df_before)+part_time_len+managers_len) 
NJ_df_per.1 = NJ_df_per.1*100
mean(NJ_df_per.1)

# Percentage full-time employees in PA 1st wave
PA_df <- df[df$d_state == 0,] # PA
PA_df_before<- PA_df$y_full.emp
part_time_len <- length(PA_df$y_part.emp) # length of variable
managers_len <- length(PA_df$y_managers) # length of variable
ompleterecords <- na.omit(PA_df_before) # delete omits
PA_df_per.1 = length(ompleterecords)/(length(PA_df_before)+part_time_len+managers_len) 
PA_df_per.1 = PA_df_per.1*100
mean(PA_df_per.1)

# Wage = $4.25 (percentage) in NJ 1st wave
NJ_df <- df[df$d_state == 1,] # NJ
NJ_df2 <- NJ_df$x_st.wage.before  #total
ompleterecords <- na.omit(NJ_df2) # delete omits
NJ_df_wage.45 <- ompleterecords[ompleterecords == 4.25]
percent_wage_45 <- (length(NJ_df_wage.45)/length(NJ_df2)) *100 # percentage

mean(NJ_df_wage.45)

# Wage = $4.25 (percentage) in PA 1st wave
PA_df <- df[df$d_state == 0,] # NJ 
PA_df2 <- PA_df$x_st.wage.before  #total
ompleterecords <- na.omit(PA_df2) # delete omits
PA_df_wage.45 <- ompleterecords[ompleterecords == 4.25]
percent_wage_45 <- (length(PA_df_wage.45)/length(PA_df2)) *100 # percentage
...
NJ.after <- mean(NJ_df$y_fte.after, na.rm=TRUE)# FTE 2nd wave NJ 
PA.after <- mean(PA_df$y_fte.after, na.rm=TRUE)# FTE 2nd wave PA 
NJ.after_wage <- mean(NJ_df$x_st.wage.after, na.rm=TRUE) # Starting wage 2nd wave NJ
PA.after <- mean(PA_df$x_st.wage.after, na.rm=TRUE)# Starting wage 2nd wave PA 
NJ.after_openday <- mean(NJ_df$x_open.per.day.sec, na.rm=TRUE) # Open hours 2nd wave NJ
PA.after_openday <- mean(PA_df$x_open.per.day.sec, na.rm=TRUE) # Open hours 2nd wave PA
NJ.after_bonus <- mean(NJ_df$x_special.program.sec, na.rm=TRUE)*100 # Recruiting bonus 2nd wave NJ
PA.after_bonus <- mean(PA_df$x_special.program.sec, na.rm=TRUE)*100 # Recruiting bonus 2nd wave PA
NJ.after_pfull <- mean(NJ_df$x_medium.soda.sec +NJ_df$x_small.fries.sec +NJ_df$x_entree.sec, na.rm=TRUE) # Price of full meal 2nd wave NJ
PA.after_pfull <- mean(PA_df$x_medium.soda.sec +PA_df$x_small.fries.sec +PA_df$x_entree.sec, na.rm=TRUE) # Price of full meal 2nd wave PA

###################################################################################################

# Percentage full-time employees in NJ 2nd wave
NJ_df <- df[df$d_state == 1,] # NJ
NJ_df_after<- NJ_df$y_full.emp.sec 
part_time_len <- length(NJ_df$y_part.emp.sec) # length of variable
managers_len <- length(NJ_df$y_managers.sec) # length of variable
ompleterecords <- na.omit(NJ_df_after) # delete omits
NJ_df_per.2 = length(ompleterecords)/(length(NJ_df_after)+part_time_len+managers_len) 
NJ_df_per.2 = NJ_df_per.2*100
mean(NJ_df_per.2)

# Percentage full-time employees in PA 2nd wave
PA_df <- df[df$d_state == 0,] # PA
PA_df_after<- PA_df$y_full.emp.sec 
part_time_len <- length(PA_df$y_part.emp.sec) # length of variable
managers_len <- length(PA_df$y_managers.sec) # length of variable
ompleterecords <- na.omit(PA_df_after) # delete omits
PA_df_per.2 = length(ompleterecords)/(length(PA_df_after)+part_time_len+managers_len) 
PA_df_per.2 = PA_df_per.2*100
mean(PA_df_per.2)

# Wage = $4.25 (percentage) in NJ 2nd wave
NJ_df <- df[df$d_state == 1,] # NJ
NJ_df2 <- NJ_df$x_st.wage.after  #total
ompleterecords <- na.omit(NJ_df2) # delete omits
NJ_df_wage.45 <- ompleterecords[ompleterecords == 4.25]
percent_wage_45 <- (length(NJ_df_wage.45)/length(NJ_df2)) *100 # percentage
...
# Wage = $4.25 (percentage) in PA 2nd wave
PA_df <- df[df$d_state == 0,] # PA 
PA_df2 <- PA_df$x_st.wage.after  #total
ompleterecords <- na.omit(PA_df2) # delete omits
PA_df_wage.45 <- ompleterecords[ompleterecords == 4.25]
percent_wage_45 <- (length(PA_df_wage.45)/length(PA_df2)) *100 # percentage
...
# Wage = $5.05 (percentage) in NJ 2nd wave
NJ_df <- df[df$d_state == 1,] # NJ
NJ_df2 <- NJ_df$x_st.wage.after  #total
ompleterecords <- na.omit(NJ_df2) # delete omits
NJ_df_wage.50 <- ompleterecords[ompleterecords == 5.05]
percent_wage.50 <- (length(NJ_df_wage.50)/length(NJ_df2)) *100 # percentage

mean(NJ_df_wage.50)

# Wage = $5.05 (percentage) in PA 2nd wave
PA_df <- df[df$d_state == 0,] # PA 
PA_df2 <- PA_df$x_st.wage.after  #total
ompleterecords <- na.omit(PA_df2) # delete omits
PA_df_wage.50 <- ompleterecords[ompleterecords == 5.05]
percent_wage.50 <- (length(PA_df_wage.50)/length(PA_df2)) *100 # percentage
...
p.before <- plot_ly(alpha = 0.5) %>% # how Opacity
  ...
  layout(barmode = "group", title = "February 1992",
           xaxis = list(tickvals=seq(4.25, 5.55, 0.1), # tick values of the x-axis
                        title = "Wage in $ per hour", # add title
                        tickangle = 90), # angle of the tick
           yaxis = list(range = c(0, 100)), # tick values of the y-axis
                      margin = list(b = 100, 
                          l = 80, 
                          r = 80, 
                          t = 80, 
...
resultsc.r2.c4 <- df.closed %>%
  group_by(d_nj) %>% # group by the state
  filter(d_nj == 1 & x_st.wage.before == 4.25) %>% # filter for only new jersey and also wage = 4.25
  dplyr::select(y_fte.after.closed) %>% # select only necessary variables
  group_by(N = n()) %>% # group by number of observations
  summarize_all(funs(mean = mean, var = var, na_sum = sum(is.na(.))), na.rm = TRUE) %>% # aggregate/summarize data
  mutate(n = N - y_fte.after.closed_na_sum) %>% 
  mutate(se = sqrt(y_fte.after.closed_var/n))
...
df2$d_ini.wage.gap2[df2$d_nj == 0] <- 0
...
# Joining 5 dummy variables to one.
df2$x_type <- names(df2[13:17])[max.col(df2[13:16])]

# Change from characters to numbers
df2$x_type <- gsub("x_northeast.pa", 1, df2$x_type)
df2$x_type <- gsub("x_easton.pa", 2, df2$x_type)
df2$x_type <- gsub("x_central.j", 3, df2$x_type)
df2$x_type <- gsub("x_south.j", 4, df2$x_type)
df2$x_type <- gsub("x_shore.j", 5, df2$x_type)

# Change class from character to numeric
df2$x_type <- as.numeric(df2$x_type)

# Subset Data set
data2 <- df2 %>% dplyr::select(y_fte.before, y_fte.after, d_nj, 
                          x_store.name, x_type )

# Create a new variable which has the difference between FTE before and after the treatment

data2 <- data2 %>% mutate(y_diff = (y_fte.after - y_fte.before))

# then delete missings 
data2 <- na.omit(data2)

## Start Matching

# Define covariate matrix, outcome, treatment
  X  <- as.matrix(data2 %>% dplyr::select(x_store.name, x_type))
  Y  <- data2$y_diff # Define outcome/Response vector
  Tr  <- data2$d_nj # Define treatment vector
...
mb <- MatchBalance(Tr ~  x_store.name + x_type,
                    match.out = mgens, nboots = 10, data = data2)
  
# 4. Save results in data set to continue analysis
...
# 5. Do a Difference in Differences

summary(lm(y_diff ~ d_nj + x_type + x_store.name, data = data.matched))

#Compare to original DID
summary(lm((y_fte.before - y_fte.after) ~ d_nj + x_type + x_store.name, data = df2))

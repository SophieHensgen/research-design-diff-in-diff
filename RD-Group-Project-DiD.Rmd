...
results.r1.c4 <- df %>%
  ...
  group_by(N = n()) %>%
  summarize_all(funs(mean, var, na_sum = sum(is.na(.))), na.rm = TRUE) %>% # aggregate/summarize data
  mutate(n = N - y_full.emp.sec_na_sum) %>% 
  mutate(se = sqrt(y_full.emp_var/n))
...
results.r2.c4 <- df %>%
  ...
  mutate(n = N - y_full.emp.sec_na_sum) %>% 
  mutate(se = sqrt(y_full.emp.sec_var/n))
...
resultsb.compare.r4 <- bind_rows(resultsb.compare.r2, resultsb.compare.r1)
  #results.compare.r3 <- select(results.compare.r3, -8)
  resultsb.compare.r4 <- bind_rows(resultsb.compare.r4, resultsb.compare.r4[4,]-resultsb.compare.r4[1,])
  resultsb.compare.r4 <- bind_rows(resultsb.compare.r4, resultsb.compare.r4[5,]-resultsb.compare.r4[2,])
  resultsb.compare.r4 <- bind_rows(resultsb.compare.r4, resultsb.compare.r4[6,]-resultsb.compare.r4[3,])
  resultsb.compare.r4$group<- c()
  kable(resultsb.compare.r4, digits=2) 
...
# Difference between Low and High & Midrange and high

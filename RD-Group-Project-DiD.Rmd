...
count.w1.r3 <- count.w1.r3.c23%>% add_row(d_state = NA, n = count.w1.r3.c1, states = "all") # add new row: total interviewed
...
# drawn from the text (cannot be seen in the data set): 
# - 473 (all), 364 (nj), 109 (pa)
...
colnames(count.w2.c1) <- c("Status", "All")
...
colnames(count.w2.c2) <- c("Status", "NJ")
...
colnames(count.w2.c3) <- c("Status", "PA")

count.w2.c3$group<- c("Answered", "Closed Permanently") # add new variable with labels

# Column 1 - 3 & columns 5 - 10

count.w2 <- full_join(count.w2.c1, count.w2.c2) # joining the individual df
